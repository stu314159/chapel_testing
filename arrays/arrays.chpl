...
writeln("After assigning two element, A is: ",A);
...
proc printArr(X: [?D] real){
  ...
  forall (i,j) in D do
    X(i,j) = -X(i,j);
  writeln("after negating X within printAr, X is: \n",X,"\n");
}
...
E = C;
writeln("After assigining C to E, E's value is:\n",E,"\n");
...
const ProbSpaceSlice = ProbSpace[1..n,3..];
...
// it is reassuring that the below code works as expected
//B = -B;
//writeln("After trying to negate B again, it is:\n",B,"\n");
